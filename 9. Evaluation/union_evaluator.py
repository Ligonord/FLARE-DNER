...
rag_data = json.load(open(f'./rag/{expt_num}/{dataset}_output.json', "r", encoding="utf-8"))
print(len(vote_data), len(rag_data))
...
merge_entity_offset = []
merge_entity_text = []
for v_o, v_t, r_o, r_t in zip(vote_entity_offset, vote_entity_text, rag_entity_offset, rag_entity_text):
    for e in r_o:
        if e not in v_o:
            v_o.append(e)
    merge_entity_offset.append(v_o)
    for e in r_t:
        if e not in v_t:
            v_t.append(e)
    merge_entity_text.append(v_t)
