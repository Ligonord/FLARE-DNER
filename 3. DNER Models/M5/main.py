...
class Trainer(object):
        ...
        with torch.no_grad():
            for i, data_batch in enumerate(tqdm(data_loader)):
                ...
                if i == 0:
                    ...
                    ent_r, ent_p, ent_c, _ = utils.decode(outputs.cpu().numpy(), entity_text, length.cpu().numpy())
                total_ent_r += ent_r
                total_ent_p += ent_p
                total_ent_c += ent_c
                grid_labels = grid_labels[grid_mask2d].contiguous().view(-1)
    ...
    def predict(self, epoch, data_loader, data):
        ...
        i = 0
        with torch.no_grad():
            for j, data_batch in enumerate(tqdm(data_loader)):
                ...
                ent_r, ent_p, ent_c, decode_entities = utils.decode(outputs.cpu().numpy(), entity_text, length.cpu().numpy())

                for ents, sent in zip(decode_entities, sentence_batch):
                    sent = sent['sentence']
                    entry = {"sentence": sent, "entity": ents}
                    result.append(entry)
        ...
        with open(f"{dataset_name}_predictions.json", "w", encoding="utf-8") as f:
