...
if __name__ == "__main__":
    ...
    for root, dirs, files in os.walk(model_dir_for_test):
        for file_name in files:
            path_se = re.search(r"run-\d{8}_\d{6}-(\w{8})[\\/].*?(val_.*)", root)
            if path_se is None:
                continue
    ...
    for run_id, model_path_list in run_id2model_state_paths.items():
        ...
        for path in model_path_list:
            ...
            print(data_out_dir, exp_name,run_name,run_id,model_name)
            # save_dir = os.path.join(data_out_dir, exp_name, run_name, run_id, model_name)
            save_dir = os.path.join(data_out_dir, exp_name, model_name)
            ...
            for filename, test_data_loader in filename2test_data_loader.items():
                ...
                # save results
                # if not os.path.exists(os.path.dirname(os.path.join(save_dir, filename))):
                #     os.makedirs(os.path.dirname(os.path.join(save_dir, filename)))
                save_as_json_lines(pred_samples, os.path.join(save_dir, 'res_data.json'))
    ...
    if cal_scores:
        json.dump(
            run_id2scores,
            open(os.path.join(os.path.dirname(save_dir), 'cal_scores.json'), 'w', encoding="utf-8"),
            ensure_ascii=False
            )
