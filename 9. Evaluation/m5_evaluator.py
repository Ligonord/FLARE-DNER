...
dataset = 'share14'
if dataset == "cadec":
    gold_data = json.load(open(f"m5/{dataset}/test_data.json", "r", encoding="utf-8"))
    pred_data = json.load(open(f"m5/{dataset}/{dataset}_predictions.json", "r", encoding="utf-8"))
else:
    gold_data = json.load(open(f"m5/{dataset}_fixed/test_data.json", "r", encoding="utf-8"))
    pred_data = json.load(open(f"m5/{dataset}_fixed/{dataset}_predictions.json", "r", encoding="utf-8"))

gold_entity = [[ent["index"] for ent in g["ner"]] for g in gold_data]
pred_entity = [[ent for ent in p["entity"]] for p in pred_data]
# print(gold_entity)
# print(pred_entity)
