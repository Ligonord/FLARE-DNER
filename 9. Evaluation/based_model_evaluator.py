...
if dataset == 'cadec':
    ...
    vote_data = json.load(open(f'./format_alignment/{dataset}_ensemble.json', "r", encoding="utf-8"))
else:
    gold_data_offset = json.load(open(f"m5/{dataset}_fixed/test_data.json", "r", encoding="utf-8"))
    vote_data = json.load(open(f'./format_alignment/{dataset}_ensemble_fixed.json', "r", encoding="utf-8"))

gold_entity_offset = [[ent["index"] for ent in g["ner"]] for g in gold_data_offset]

for i in range(1, 6):
    pred_entity_offset = [[ent['index'] for ent in g[f"entity_list_{i}"]] for g in vote_data]

    print(f'Model {i}:')
    relaxed_matching(gold_entity_offset, pred_entity_offset)
    relaxed_matching_jaccard_hungarian(gold_entity_offset, pred_entity_offset)
    token_coverage(gold_entity_offset, pred_entity_offset)
    entity_f1_offset(gold_entity_offset, pred_entity_offset)
    sen_instance_f1(gold_entity_offset, pred_entity_offset)
    print()